...
import ByteStringTools (crack1Xor, crackXor, detectECB, fromB64, fromHex, padPKCS7, scorePhrase, toB64, toHex, xor)
...
case_4 :: Assertion
case_4 = do
    ss <- map fromHex . BS.lines <$> BS.readFile "case_4.txt"
...
case_7 :: Assertion
case_7 = do
    s <- fromB64 . BS.concat . BS.lines <$> BS.readFile "case_7.txt"
...
case_8 :: Assertion
case_8 = do
    ss <- map fromHex . BS.lines <$> BS.readFile "case_8.txt"
    head (filter (detectECB 16) ss) @?= result
  where
...
case_9 :: Assertion
case_9 = padPKCS7 20 s @?= result
  where
    s = "YELLOW SUBMARINE"
    result = "YELLOW SUBMARINE\EOT\EOT\EOT\EOT"
